...
## Утилита: Поиск изображений с реальными людьми

Отдельный скрипт `find_faces.py` для фильтрации изображений с реальными лицами среди скачанного контента.
...
На Windows может потребоваться предварительно:
```bash
